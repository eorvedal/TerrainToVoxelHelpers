...
namespace Fraktalia.Core.FraktaliaAttributes
{
	...
	public class TextureArrayTemplateV2 : ScriptableObject
	{
		[NonReorderable]
		[Header("Materials To Extract")]
		public Material[] Materials;
		public bool ExtractFromTerrain = false;
		public TerrainData TerrainToExtractFrom;
		[HideInInspector] public TerrainLayer[] terrainLayers;
		...
		public Texture2DArray CreateColorTextureArray(Color[] colors, string fileName, int textureSize = 1)
		{
			// Ensure minimum size
			int size = Mathf.Max(1, textureSize);

			// Create a Texture2D array
			Texture2DArray textureArray = new Texture2DArray(size, size, colors.Length, TextureFormat.RGBA32, true);
			textureArray.filterMode = FilterMode.Bilinear;
			textureArray.wrapMode = TextureWrapMode.Repeat;

			// Loop through each color and create a texture
			for (int i = 0; i < colors.Length; i++)
			{
				// Create a single-color texture
				Texture2D tempTexture = new Texture2D(size, size, TextureFormat.RGBA32, true);
				Color[] pixels = Enumerable.Repeat(colors[i], size * size).ToArray();
				tempTexture.SetPixels(pixels);
				tempTexture.Apply();
			...
			Debug.Log("Saved Color TextureArray to " + path);
		...
		public void CreateAllTextureArrays()
		{
			...
			Texture2DArray baseMultiplierArray = CreateColorTextureArray(BaseTextureMultiplier, FinalName + "_BaseMultiplier");
			Texture2DArray metallicMultiplierArray = CreateColorTextureArray(MetallicMultiplier, FinalName + "_MetallicMultiplier");
			Texture2DArray emissionMultiplierArray = CreateColorTextureArray(EmissionMultiplier, FinalName + "_EmissionMultiplier");
			AssetDatabase.SaveAssets();
			AssetDatabase.Refresh();
		...
		public void SaveProfile()
		{
			...
			TextureArrayTemplateV2 duplicate = Instantiate(this);
		...
		public void ExtractMaterialsFromTerrain()
		{
			if (TerrainToExtractFrom == null)
			{
				Debug.LogError("TerrainToExtractFrom is not assigned!");
				return;
			}

			terrainLayers = TerrainToExtractFrom.terrainLayers;

			OcclusionTextures = new Texture2D[terrainLayers.Length];
			DiffuseTextures = new Texture2D[terrainLayers.Length];
			NormalTextures = new Texture2D[terrainLayers.Length];
			EmmissiveTextures = new Texture2D[terrainLayers.Length];
			HeightTextures = new Texture2D[terrainLayers.Length];
			MetallicTextures = new Texture2D[terrainLayers.Length];

			BaseTextureMultiplier = new Color[terrainLayers.Length];
			MetallicMultiplier = new Color[terrainLayers.Length];
			EmissionMultiplier = new Color[terrainLayers.Length];
			SmoothnessTextureSource = new SmoothnessSource[terrainLayers.Length];
			...
			for (int i = 0; i < terrainLayers.Length; i++)
			{
				...
				{
					DiffuseTextures[i] = terrainLayers[i].diffuseTexture != null ? terrainLayers[i].diffuseTexture : WhiteDefaultTexture;
					NormalTextures[i] = terrainLayers[i].normalMapTexture != null ? terrainLayers[i].normalMapTexture : NormalDefaultTexture;
					OcclusionTextures[i] = terrainLayers[i].maskMapTexture != null ? terrainLayers[i].maskMapTexture : WhiteDefaultTexture;
					EmmissiveTextures[i] = BlackDefaultTexture;
					HeightTextures[i] = HeightDefaultTexture;
					MetallicTextures[i] = WhiteDefaultTexture;
